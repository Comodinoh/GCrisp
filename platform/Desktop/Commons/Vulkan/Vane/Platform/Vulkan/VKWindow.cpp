...
#include <Vane/Events/KeyEvent.h>
...
static void GLFWErrorCallback(int error, const char* desc) {
    VANE_CORE_ERROR("GLFW Error: {0}: {1}", error, desc);
}
...
void VKWindow::Init() {
    ...
    VANE_CORE_INFO("Creating GLFW window {0} ({1}, {2})", data.Title, data.Width,
                 data.Height);

    if (!s_GLFWInitialized) {
        VANE_PROFILE_SCOPE("glfwInit - OpenVKWindow");
        int success = glfwInit();
        VANE_CORE_ASSERT(success, "Could not initialize GLFW!");
    ...
    {
        VANE_PROFILE_SCOPE("glfwCreateWindow - OpenVKWindow");
    ...
    VANE_CORE_VERIFY(vkCreateInstance(&info, nullptr, &s_VulkanInstance) ==
                       VK_SUCCESS,
    ...
    VANE_CORE_INFO("Vulkan Specifications:");
    VANE_CORE_INFO("   Vendor: {0}", m_GraphicsSpec.Vendor);
    VANE_CORE_INFO("   Renderer: {0}", m_GraphicsSpec.Renderer);
    VANE_CORE_INFO("   Vulkan Version: {0}", m_GraphicsSpec.APIVersion.GetName());
    VANE_CORE_INFO("   SL Version: {0}", m_GraphicsSpec.SLVersion);
