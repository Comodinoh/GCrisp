...
GLShader::GLShader(const ShaderSpecification& spec) {
    ...
    for (auto&& [type, source] : spec.shaders) {
        VANE_CORE_ASSERT(!StringFromShaderType(type).empty(),
                       "ShaderType string cannot be empty!");
        ...
        if (status == GL_FALSE) {
            ...
            VANE_CORE_ERROR(
                "Failed to compile {0} shader. Please refer to the logs below:",
                StringFromShaderType(type));
            VANE_CORE_ERROR(std::string(infoLog));
            VANE_CORE_ASSERT(false, "Error occurred during shader compilation!");
            return;
        }
    ...
    if (status == GL_FALSE) {
        ...
        VANE_CORE_ERROR(std::string(infoLog));
        VANE_CORE_ASSERT(false, "Failed to link shader program!");
        return;
    }
