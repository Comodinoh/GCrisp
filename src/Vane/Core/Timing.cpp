...
void TimingsProfiler::Shutdown() {
    for (auto& pair : s_ProfilerMap) {
        ...
        VANE_CORE_VERIFY(output.is_open(),
        ...
        VANE_CORE_ASSERT(!results.empty(), "Results is empty!");
        size_t i = 0;
...
void TimingsProfiler::StartProfiler(const char* file) {
    ...
    // VANE_CORE_VERIFY(iter == s_ProfilerMap.end(), "There is already a profiler
