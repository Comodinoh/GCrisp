...
void* Allocator::Allocate(size_t size)
{
    ...
    VANE_CORE_TRACE("Allocated {0}", m_Statistics.Allocated);
...
bool Allocator::Free(void* data)
{
    ...
    if (iter == m_Data.Allocated.end()) {
        VANE_CORE_WARN("Freeing not registered memory address {0}", data);
    } else {
    ...
    VANE_CORE_TRACE("Freed {0}", m_Statistics.Freed);
