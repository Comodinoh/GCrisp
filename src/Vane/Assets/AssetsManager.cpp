...
#include "Vane/Renderer/Utils.h"

#include <stb_image.h>

#include "Vane/Core/UUID.h"
...
AssetMetadata& AssetsManager::FetchMetadata(const AssetID& asset) {
    ...
    VANE_CORE_ASSERT(iter != m_AssetRegistry.end(),
...
AssetID AssetsManager::LoadAsset(const AssetMetadata& metadata) {
    ...
    // {
    //     VANE_CORE_TRACE("AssetsManager::LoadAsset - Waiting for valid asset");
    //     uuid = AssetID();
    // }
...
Reference<Asset>
AssetsManager::ImportAsset(const AssetMetadata& metadata) const {
    ...
    case AssetType::Unknown: {
        VANE_CORE_ASSERT(false, "Unknown asset type!");
    }
    }
    return nullptr;
}
...
Graphics::Texture2D*
AssetsManager::LoadTexture2D(const std::string& name) const {
    VANE_PROFILE_FUNC();
    ...
    VANE_CORE_VERIFY(data, "Failed to load '{0}' image!", name);

    return Graphics::Texture2D::Create(
        {data, {width, height}, (uint32_t)channels});
}

Graphics::Texture2D* AssetsManager::LoadRawTexture2D(
    const Graphics::TextureSpecification& spec) const {
    VANE_PROFILE_FUNC();
    VANE_CORE_ASSERT(spec.Data, "Provided null image to load!");
...
Graphics::Shader* AssetsManager::LoadShader(const std::string& name) const {
    ...
    if (!file.is_open()) {
        VANE_CORE_ERROR("Could not open file '{0}'", path);
        return nullptr;
    }
    ...
    if (path.ends_with(".glsl")) {
        ...
        while (pos != std::string::npos) {
            ...
            VANE_CORE_ASSERT(nextPos != std::string::npos,
        ...
        if (path.ends_with(".vert")) {
            shader =
                Graphics::Shader::Create({{Graphics::SHADER_VERTEX, source}});
        } else if (path.ends_with(".frag")) {
            shader =
                Graphics::Shader::Create({{Graphics::SHADER_FRAGMENT, source}});
        } else {
            VANE_CORE_ERROR(
