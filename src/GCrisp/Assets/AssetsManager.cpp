...
namespace GCrisp
{
    ...
    AssetMetadata& AssetsManager::FetchMetadata(const AssetID& asset)
    {
        ...
        GC_CORE_ASSERT(iter != m_AssetRegistry.end(), "Non existent asset id in registry");
    ...
    AssetID AssetsManager::LoadAsset(const AssetMetadata& metadata)
    {
        ...
        GC_CORE_TRACE("AssetsManager::LoadAsset - Loaded Asset with ID: {0}, Path: {1} and Type: {2}", uuid.Get(), metadata.Path, StringFromAssetType(metadata.Type));
    ...
    Reference<Asset> AssetsManager::ImportAsset(const AssetMetadata& metadata) const
    {
        ...
        case AssetType::Shader:
            {
                // TODO: Add default debug shader when loaded shader isn't valid (maybe) or crash the program
    ...
    void AssetsManager::FetchAssetInternal(const AssetID& id, Reference<Asset>& asset)
    {
        GC_PROFILE_FUNC();
        if (!IsAssetValid(id))
        {
    ...
    Graphics::Shader* AssetsManager::LoadShader(const std::string& name) const
    {
        ...
        if (path.ends_with(".glsl"))
        {
            ...
            {
                GC_CORE_ERROR("Could not parse shader '{0}' type. Please add '//type [shader/fragment]' at the start of your shader", path);
                return nullptr;
            }
            int i = 0;
            int sizeI = 0;
