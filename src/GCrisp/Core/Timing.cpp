...
namespace GCrisp
{
    ...
    {
        GC_PROFILE_FUNC();
        elapsed.m_Time = duration_cast<microseconds>(end - m_Start).count() / 1000000.0f;
        m_Start = end;
    }

    ScopedTimer::~ScopedTimer()
    {
        m_Elapsed = duration_cast<microseconds>(steady_clock::now() - m_Start).count() / 1000000.0f;
    }

    std::unordered_map<std::string, std::vector<ProfileResult>> TimingsProfiler::s_ProfilerMap =
    ...
    void TimingsProfiler::Init()
    {
        s_ProfilerMap.reserve(3);
    }
    ...
    void TimingsProfiler::Shutdown()
    {
        for (auto& pair : s_ProfilerMap)
        {
            ...
            GC_CORE_VERIFY(output.is_open(), "Could not open output {0} profiler file!", profiler);
            ...
            for (auto& [Name, Thread, Elapsed, Start] : results)
            {
                ...
                data << "\"pid\":0,\n";
                data << "\"tid\":" << Thread << ",\n";
                data << "\"ts\":" << std::chrono::duration<double, std::micro>(Start.time_since_epoch()).count();
                data << "}";
                if (i != results.size() - 1)
                    data << ",";
                data << "\n";
    ...
    void TimingsProfiler::AddProfileResult(ProfileResult& result)
    {
        ...
        s_ProfilerMap[s_CurrentProfiler].emplace_back(result.Name, result.Thread, result.Elapsed, result.Start);
    }
}
