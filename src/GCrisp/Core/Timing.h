...
class Timer
{
  public:
	...
	void ProcessTime(ProcessedTime&					 elapsed,
...
{
  public:
	ScopedTimer(steady_clock::time_point& start, ProcessedTime& elapsed)
		: m_Elapsed(elapsed), m_Start(start)
	{
	}
...
{
  public:
	ProfilerTimer(ProfileResult& result)
		: ScopedTimer(result.Start, result.Elapsed), m_Result(result)
	{
	}
...
class TimingsProfiler
{
  ...
  private:
	static std::unordered_map<std::string, std::vector<ProfileResult>>
				 s_ProfilerMap;
