...
void AssetsManager::LoadShader(const std::string& name) 
{
  ...
  if(path.ends_with(".glsl"))
  {
    ...
    while(pos != std::string::npos)
    {
      ...
      pairs[i] = {formattedType, (pos == std::string::npos) ? source.substr(nextPos) : source.substr(nextPos, pos-nextPos)};
      i++;
    }
  ...
  }else
  {
    ...
    {
      GC_CORE_ERROR("Could not identify shader called '{0}' with no correct extension! ('.glsl', '.vert', '.frag')");
